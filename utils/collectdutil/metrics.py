...
class Metric(object):
    ...
    def __str__(self):
        attrs = ('type_instance', 'type', 'value', 'plugin', 'plugin_instance', 'time', 'encoded_dimensions',
                 'interval', 'host')
        items = ['{}: {}'.format(attr, str(getattr(self, attr))) for attr in attrs]
        return 'Metric({})\n'.format(', '.join(items))
...
def encode_dimensions(dimensions=None, max_len=plugin_instance_max_length - 2):
    ...
    """
    if dimensions:
        encoded_dimensions = ','.join(['='.join(p) for p in dimensions.items()])
        if len(encoded_dimensions) > max_len:
            collectd.warning('Truncating encoded dimensions: {}'.format(encoded_dimensions))
